...
Node.prototype.startWeb3Connection = async function()
{
	...
	// web3 = new Web3('http://' + (process.env.RPC_HOST || 'localhost') + ':' + (process.env.RPC_PORT || '8545'));
	const nodeURL = process.env.NODE_URL || 'wss://localhost:8546'
...
Node.prototype.checkWeb3Connection = async function()
{
	...
	if (!this._web3)
	{
		...
		try {
			isConnected = await web3.eth.getProtocolVersion()
		}
		catch(e){
			console.error(e)
		}
...
Node.prototype.reconnectWeb3 = async function()
{
...
Node.prototype.getInfo = async function()
{
	...
	try {
		this.info.coinbase = await web3.eth.getCoinbase();
		this.info.node = await web3.eth.getNodeInfo();
		this.info.net = await web3.eth.getChainId();
		this.info.protocol = web3.toDecimal(await getProtocolVersion());
		this.info.api = await web3.version.api();
...
Node.prototype.setFilters = async function()
{
	var self = this;
	...
	try {
		...
		await web3.eth.subscribe('newBlockHeaders', async function (err, data)
		{
			let hash
			var now = _.now();
			...
			{
				console.error(err)
				hash = await web3.eth.getBlockNumber();
			}
			else{
				hash = data.hash
			}
	...
	// 	web3.eth.subscribe('pendingTransactions', function (err, hash)
	// 	{
	// 		var now = _.now();
